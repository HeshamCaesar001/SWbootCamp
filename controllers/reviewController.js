...
//@description    Add review fo a bootcamp
...
exports.updateReview = asyncHandler(async (req, res, next) => {
  ...
  // make sure review belong to loggedin user or admin
  if (review.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `user with ${req.user.role} role can not update this review`,
        401
      )
    );
  }
...
exports.deleteReview = asyncHandler(async (req, res, next) => {
  ...
  if (review.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `user with ${req.user.role} role can not delete this review`,
        401
      )
    );
  }
